...
# LIGHT COUPLING
# =========================================================
gamma = 0.005  # keep 0 if you truly want "no light effect at all"
...
# =========================================================
# IRREGULAR SLEEP–WAKE LIGHT SCHEDULE
# =========================================================
rng_sleep = np.random.default_rng(seed=42)

sleep_windows = []

n_days = int(total_time // 24)

for day in range(n_days):
    # Bedtime uniformly between 23:00 and 03:00
    bedtime = rng_sleep.uniform(23, 27)
    if bedtime >= 24:
        bedtime -= 24

    sleep_duration = rng_sleep.uniform(6, 9)

    sleep_start = day * 24 + bedtime
    sleep_end = sleep_start + sleep_duration

    sleep_windows.append((sleep_start, sleep_end))


def L_sleep(t):
    """
    Light = 0 during sleep
    Light = 1 when awake
    """
    for t0, t1 in sleep_windows:
        if t0 <= t <= t1:
            return 0.0
    return 1.0
...
plt.tight_layout()
plt.savefig("Final_plots/figures/timeseries_student.png", dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig("Final_plots/figures/phase_student.png", dpi=300, bbox_inches='tight')
plt.show()
